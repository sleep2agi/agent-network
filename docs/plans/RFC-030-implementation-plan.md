...
| Historical server test isolation | **In progress / separate PR** | Baseline's eight module-singleton server/DB/port failures are owned by 通信测试马 in [#434](https://github.com/sleep2agi/agent-network/issues/434). Work resumed after #435 passed; canonical Bun runner must unset `DATABASE_URL`, isolate DB/port/process per manifest entry and run green three times |
