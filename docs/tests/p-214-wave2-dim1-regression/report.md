...
- log tail:
```
[anet] Starting Dashboard on dbe1a4cdb488:3000...
...
[anet] note: first launch compiles Next.js routes — expect 30-60s before http://dbe1a4cdb488:3000 responds.
```
...
```
  ...
  User: admin (u_264b81e6f149)
  Role: admin
  Hub:  http://127.0.0.1:9200

  Networks:
    default (net_842e9e33) ← current
...
↑↓ navigate • ⏎ select[?25l[23Gspawn anet node create my-bot
? 选择 runtime:
❯ claude-agent-sdk — 任意 OpenAI/Anthropic-compat vendor (intern / MiniMax /
Claude / GLM / ...)
  claude-code-cli  — Anthropic Claude (Max/Pro plan), 复用 `claude` CLI 登录态
...
- ✅ `SSE connected` visible in log: `[03:55:18] [INFO ] [my-bot] SSE connected`
- start log tail:
```
...
[03:55:17] [INFO ] [my-bot] 启动
[03:55:17] [INFO ] [my-bot]   alias:   my-bot [from: --alias flag]
[03:55:17] [INFO ] [my-bot]   runtime: grok-build-acp
[03:55:17] [INFO ] [my-bot]   model:   grok-build (default)
[03:55:17] [INFO ] [my-bot]   hub:     http://127.0.0.1:9200 (auth)
[03:55:17] [INFO ] [my-bot]   user:    admin (admin)
[03:55:17] [INFO ] [my-bot]   network: default
[03:55:17] [INFO ] [my-bot]   tools:   all (Claude Code preset — built-in: WebFetch/WebSearch/Bash/Read/Write/Edit/Glob/Grep/Task/...)
[03:55:17] [INFO ] [my-bot]   channels: (none)
[03:55:17] [INFO ] [my-bot]   session: (new)
[03:55:17] [INFO ] [my-bot]   log-dir: /work/.anet/nodes/my-bot/logs
[03:55:17] [INFO ] [my-bot]   goals:   /work/.anet/nodes/my-bot/goals.json
[03:55:18] [INFO ] [my-bot] 已注册到 CommHub
[03:55:18] [INFO ] [my-bot] SSE connected
```
...
- 维度1/Step1/anet -v 输出非常规/级别 P3/用户预期 'vX.Y.Z'。实际 'anet v2.2.12-preview.0Components (auto-fetched on first use, you don't need to install them manually):  ○ agent-node — not installed yet (will fetch via npx on first use)  ○ commhub-server — not installed yet (will fetch via npx on first use)Optional runtimes (install only what you'll use):  ○ claude CLI — only needed for the claude-code-cli runtime  ○ codex CLI — only needed for the codex-sdk runtimeNothing is broken — components are fetched the first time you run:  anet hub start          # bootstraps commhub-server  anet node start <name>  # bootstraps agent-nodeDocs: https://anet.sh/guide/getting-started'。
