...
>
> Earlier versions had a separate failure mode where a stale `.mcp.json` left in the project directory (written by an older `anet` setup flow with a different alias) would be picked up by Grok as a fallback, attaching the **wrong CommHub alias** to outbound `send_task` calls. **Fixed in `agent-node@2.4.7-preview.X` (currently `2.4.7-preview.3`)** (v0.10.11 preview, commit [`4b5a657`](https://github.com/sleep2agi/agent-network/commit/4b5a657)): the `grok-build-acp` runtime now passes `mcpServers` explicitly on every `session/new` / `session/load`, so Grok never falls back to reading cwd `.mcp.json`. Stale-file identity is now **structurally impossible**. Once v0.10.11 is promoted to `@latest`, this caveat is gone for stable users too — until then, stable v0.10.10 users (`agent-node@2.4.6`) still see the old behavior and should delete any stale `.mcp.json` in the cwd before starting a `grok-build-acp` node.

If the project directory carries an older `.anet/node-server.js`, Grok may receive a stale CommHub tool schema that is missing `get_task` / `parent_task_id` — explicit delegation chains will silently break. (This is independent of the `.mcp.json` issue above — the `.mcp.json` part is structurally fixed in v0.10.11 preview; this `.anet/node-server.js` helper script is regenerated by `agent-node` itself and **not** affected by `#204`.)
