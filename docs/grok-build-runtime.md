...
> **`.mcp.json` shared-identity issue — RESOLVED in v0.10.11 preview ([#204](https://github.com/sleep2agi/agent-network/issues/204))**
>
> Earlier versions had a separate failure mode where a stale `.mcp.json` left in the project directory (written by an older `anet` setup flow with a different alias) would be picked up by Grok as a fallback, attaching the **wrong CommHub alias** to outbound `send_task` calls. **Fixed across the preview.2 → preview.7 chain in `agent-node@2.4.7-preview.X`** (v0.10.11 preview, root-cause commit [`4b5a657`](https://github.com/sleep2agi/agent-network/commit/4b5a657)):
>
...
> - **preview.7** ([`72e28fd`](https://github.com/sleep2agi/agent-network/commit/72e28fd)): **per-node isolated cwd** to defeat stale `.mcp.json` discovery. Vincent UAT after preview.6 still showed `from=grok测试员` on receiver — root cause: Grok CLI auto-reads cwd `.mcp.json` **alongside** the ACP `session/new` mcpServers injection, so two commhub MCP servers coexist and the stale stdio one wins the LLM's hello message. preview.7 fix: pass per-node isolated dir `<home>/.anet/nodes/<node-id>/grok-cwd/` via ACP `session/new`'s existing `cwd` field. Per-node dir mirrors top-level user files via symlink (so LLM `Read('./README.md')` still works) but **skips `.mcp.json`** so cwd discovery finds nothing. Multi-node concurrent-spawn safe by construction; fallback to user cwd on mkdir/readdir failure (preview.6 behavior, no regression).
>
> Once v0.10.11 is promoted to `@latest`, this caveat is gone for stable users too — until then, stable v0.10.10 users (`agent-node@2.4.6`) still see the legacy stdio behavior and should delete any stale `.mcp.json` in the cwd before starting a `grok-build-acp` node.

If the project directory carries an older `.anet/node-server.js`, **legacy `grok-build-acp` nodes** (v0.10.10 stable, `agent-node@2.4.6`) may receive a stale CommHub tool schema that is missing `get_task` / `parent_task_id` — explicit delegation chains will silently break. (This is independent of the `.mcp.json` issue above. v0.10.11 preview.6 onward sidesteps it entirely by switching to HTTP transport — no `.anet/node-server.js` is spawned. The `.anet/node-server.js` helper script is regenerated by `agent-node` itself and is **not** affected by `#204`.)
