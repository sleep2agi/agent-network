...
### Stats
- VitePress docs site: anet.sh (Chinese + English)
- Verified case/demo pages only (debate, hello-world, translation-pipeline, telegram-squad)
- npm homepage → anet.sh, repo URL → agent-network

### Added since V3.14
- **Docs: Account System page** (中英双语) — registration, login, RBAC, Agent tokens, model API keys
- **Docs: Cases module** — only verified runnable cases remain; manual-only case pages are removed until matching Docker assets/tests exist
...
- **npm: homepage** → https://anet.sh for all 3 packages
