...
3. **bunx 缓存老版 `commhub-server`。** CLI 通过 `PINNED_SERVER_VERSION`（`bin/cli.ts:2125`）来规避 bunx 缓存。v0.8 上线时，老 CLI 用户会拉到老 server。需要协调好版本 bump 节奏，或者接受一个 release 窗口期内老 server 仍接受 master token —— 反正 v0.8 的软废弃就是给这个 case 准备的。
