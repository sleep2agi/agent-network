...
### 3.5 setup wizard prompt 加 codex-code-cli 选项
...
实施 §3 全部设计，cli.ts 改动 ~50-80 行，加 Docker E2E test。

**优点**：
- 跟 claude-code-cli 完全对称，用户体验一致
- Vincent 自己半成品 config 直接替换为产品化 runtime
- RFC-002 Phase 2 channel binding 路径打通

**预算**：通信工程马 实施 1-2 天 + 通信SDK马 review。

### B — 仅 docs 教用户自配

不动 cli.ts，写 docs guide 教用户手配 `~/.codex/config.toml + [mcp_servers.commhub]` + 手 spawn codex。

**优点**：零代码改动
**缺点**：用户每次手动；跨 runtime 体验不一致；anet CLI 仍缺这一类 runtime

### C — 暂不加，等 codex 用户起来再做

先观察 codex CLI 用户增长曲线，等需求明显再实施。

**优点**：scope creep 防控
**缺点**：Vincent 自己已经在用 — 需求已 validate；机会成本 / 用户拉新错失

### 我的倾向（不替 Vincent 拍板）

**A** — Vincent stale config 是实证需求 + 跟 claude-code-cli 对称是合理产品诉求 + cli.ts 改动量可控（~50-80 行）+ 不动其他业务码。但 B/C 决策权归 Vincent。
