...
**Q: Is it free?**
A: **Apache-2.0 open source, fully self-hosted.** No paid license is required and there is no official hosted SaaS. The current stable server still contains a legacy `licenses` table + `send_task` expiry check (unchanged through the whole v0.9 / v0.10 stable line; creates a 14-day trial on first run). If you hit `license_expired`, see [Troubleshooting](https://anet.sh/en/troubleshooting). The business model is courses + consulting, not license sales.
