...
> **R220 校准（2026-05-13）**：本节的「内置轻量 UI」+「`http://YOUR_IP:9200/dashboard`」是 V2 早期设计草稿，**v0.8 实际未实现** —— commhub-server `server/src/index.ts` 没有 `/dashboard` 路由（`server/src/index.ts` 里 `/dashboard` 0 hit；[`server/src/server.ts`](https://github.com/sleep2agi/agent-network/blob/main/server/src/server.ts) 里有且只有 1 处，是第 287 行的注释 `any future HEAD/Range/dashboard-proxy`，不是路由 —— 写清楚是因为下一个人 grep 到它会以为这句话错了）。当前**唯一 Dashboard 是独立的 Next.js 包 `@sleep2agi/agent-network-dashboard`**，通过 `anet hub dashboard` 子命令拉起（[`agent-network/bin/cli.ts:7757`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L7757) `sub === "dashboard"` 分支，默认端口 3000；版本不再 hardcode pin —— [`dashboardReleaseTag()` cli.ts:2489](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L2489) 默认拉 `@preview` tag，可用 `ANET_DASHBOARD_VERSION` env 覆盖，跟 anet release channel 对齐 — 见 #61）。🔴 上面两个行号锚在 2026-08-28 修过一次：`dashboardReleaseTag()` 2452→2489（本次改动造成），`sub === "dashboard"` 7326→7757（**在此之前就已漂了 394 行**，`scripts/check-doc-symbol-pins.py` 因为「锚文本没点名符号」把它归进「跳过」，而该脚本自己在输出里写着**跳过的不是通过**）。最新部署方式见 [anet.sh/guide/dashboard](https://anet.sh/guide/dashboard)。下面的「两种 Dashboard」/「内置 UI 设计原则」/「实现方案」/「HTML 结构」全是 V2 设计草稿，仅保留历史背景，**当前不适用**。
