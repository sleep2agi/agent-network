...
verify [`cli.ts:229 loadProfile`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L229):
```ts
...
verify [`cli.ts:247-265 saveProfile`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L247):
```ts
...
`anet node start` 检测到 `runtime: "claude-code-cli"` 时，自动确保 Channel 插件可用（[`cli.ts:1609 ensureMcpJson`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1609)）：
...
3. 写入 `.mcp.json`：`commhub → .anet/node-server.js`（[cli.ts:1689](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1689)）
...
已配置过且内容一致直接跳过（compare-by-content：`if (src !== dst) writeFileSync(...)`，[cli.ts:1644-1645](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1644)）。`anet init project` 也做同样的事（另外还写 CLAUDE.md）。
