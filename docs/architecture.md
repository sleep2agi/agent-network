...
`anet node start` 检测到 `runtime: "claude-code-cli"` 时，自动确保 Channel 插件可用（[`cli.ts:1607 ensureMcpJson`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1607)）：
...
3. 写入 `.mcp.json`：`commhub → .anet/node-server.js`（[cli.ts:1687](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1687)）
...
已配置过且内容一致直接跳过（compare-by-content：`if (src !== dst) writeFileSync(...)`，[cli.ts:1642-1643](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1642)）。`anet init project` 也做同样的事（另外还写 CLAUDE.md）。
...
方式 C: anet run 独立 SSE Agent（minimal, V2 兼容）
  ...
  仍在 cli.ts:2007 (runCommand) 注册, 但 V3 推荐用方式 B（anet node create）
```
