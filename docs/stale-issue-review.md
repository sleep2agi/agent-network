...
## 🔴 而且不只是「陈旧」的 —— **今天刚更新过的 issue 同样可能已经做完**

这份文档的出发点是「30 个超过 30 天没动」。2026-08-31 一天里我撞到 **11 条**
「正文说的事在 `origin/main` 上已完成或部分完成」的 issue，而它们**一条都不陈旧**：

```
#1545 #1650 #1635 #1606 #1548 #1571 #1563 #1615 #1627 #1634 #1645
全部建于 08-30（一天前）
其中 9 条的最后更新就在 08-31 当天（最近的距我查时不到 3 小时）
```
...
| | 陈旧 issue | 这 11 条 |
|---|---|---|
| 问题出在 | 做完了没人回来关 | **main 推进得比 issue 状态快** |
| 时间信号 | 弱，但至少存在（久没动会让人警觉） | **反向**：`updatedAt` 越新，越像「这就是现状」 |

⇒ **一个刚刚更新过的 issue，比一个三个月没动的更容易骗人。** 三个月没动的会让人
先去查一遍；今天更新过的会被直接当成当前状态读下去。

判据不是 `updatedAt`，是 **`origin/main` 的现况**。所以本文档「步骤」里的那几步
（先读正文要什么 → 再逐条对 main）**对新 issue 同样必须走一遍**，不能因为它是昨天
建的、今天还在讨论就跳过。

实测代价：那 11 条里有 10 条是我**动手前**查出来的（每条省一轮）；只有 1 条
（#1627）是我**写完代码、跑完测试、做完两向见证之后**才发现「这件事今天早些时候
已经由 #1633 做掉了」—— 代价三轮。**差别只在于有没有在动笔前问一句「它现在是什么样」。**
...
### 6b. 「注释里提到编号」和「测试名里带编号」是两个强度完全不同的信号

第 6 节说去查代码里指向这条 issue 的注释。**但那个信号很松。**
2026-08-31 对全部 120 条 OPEN issue 实测:

```
宽判据(编号出现在任何 .ts/.py/.sh 里)      53 条
严判据(出现在 *.test.ts 的 describe/test 名里)  15 条
```
...
|---|---|---|
| `#928` | `#   见 #928。` | ❌ 只是引用背景 |
| `#1092` | `## 背景(#1092)` | ⚠️ 因它而建的门,可能只修了一半 |
| `#881` | `describe("tainted composer diagnostics (issue #881)")` | ✅ 回归测试,强信号 |

⇒ **找 `*.test.ts` 里 `describe(...)` / `test(...)` 名字带编号的**,那是有人为这条
issue 写了回归测试;注释里的 `见 #NNNN` 只说明"这里和它有关"。

### 6c. 🔴 有回归测试**也不足以关** —— 还隔着「修复到没到用户」

上面那 15 条严判据命中里,**至少 3 条不能关**,而且原因都不是"没修":

| issue | 代码状态 | 为什么仍不能关 |
|---|---|---|
| `#1698` | 有 5 处测试 | 在等 owner 决定 flag 命名 —— **产品决策未定** |
| `#1548` | 有 3 处测试 | 修复在 `agent-node .56`,而 npm `latest` 停在 `.34` —— **没到用户** |
| `#1595` | 有 1 处测试 | 生产 hub 仍是 `.38` —— **等升级 GO** |

**「main 上修好了」和「这条 issue 可以关了」之间,隔着一次发布或一个决策。**
判「已修」时必须同时回答:**修在哪个 ref?离用户还有几步?**
(同族:`docs/RELEASE-SOP.md` 的两阶段规则 —— 发 preview 与推 latest 是两件事。)

🔴 实测里这一类占比不低:今晚追的五条用户可见问题(`#1548` / `#1433` /
`#1639` / `app#196` / dashboard 未读徽标),**代码层面全部已修**,
卡的全是"把修好的东西送出去"那一步。
⇒ **一条 issue 长期 OPEN,常常不是没人做,是它在等发布。**
这种情况该做的是**把它挂到那个发布动作上**,而不是关掉它。
...
```doc-claims
server/src/db.ts :: 427 :: ADD COLUMN team
...
正文里的引用是**裸文件名**:`cli.ts:3450`、`db.ts:427`。而 `cli.ts` 在
...
- **不检查正文的结论对不对。** 「`db.ts:427` 有 `ADD COLUMN team`」成立,不代表「#175 已交付」成立 —— 后者要人读 issue 正文。
- **不检查引用之外的散文。**
...
| issue | 结论 | 决定性证据 |
|---|---|---|
| #175 node.team | **部分核验** | `db.ts:427` 有 `ALTER TABLE nodes ADD COLUMN team TEXT`;`api-nodes-shape.test.ts` 把 `team` 写进 `/api/nodes` 投影断言。**只覆盖标题那一句;正文的「详细方案」未逐条比对,因此不建议据此关闭** |
