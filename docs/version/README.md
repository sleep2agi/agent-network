...
| **v0.11.0** | 迭代中（preview 线） | 2.3.0（现 preview.33+） | 2.5.0（现 preview.25+） | 0.9.0（现 preview） | 0.7.0（现 0.6.3-preview） | [plan](./0.11.0/) |

## 规则

0. **正式版本（latest）保证稳定**：只包含功能盘点为 ✅ 的能力；⚠️ 必须在文档明确标坑；❌ 一律圈围在 preview、不进 latest。latest 用户不踩暗坑是底线。
