...
export interface CodexGatewayAssemblyOptions {
  ...
  log?: (msg: string) => void;
  /**
   * Frozen A `GatewayLifecycleOptions.backendCapability` — a stable
   * ≥32-char capability string surfaced in responses. Callers should
   * pass a durable identifier for the anet node's backend surface
   * (e.g. `anet-codex-gateway:v1:<node_alias>`); Phase 1 tests pass
   * a fixed test constant. Required by frozen A contract.
   */
...
export async function assembleCodexGateway(
  ...
  const scheduler = new GatewayScheduler({
    ...
    // SEAM: A's `GatewayLifecycle` keeps `HumanOwnerCoordinator` private
    // (Commit 1/2 frozen). Public accessor is `humanOwnerAttached(): boolean`.
    // Scheduler polls this on each dispatch decision — no observer needed.
  ...
  // SEAM: A's GatewayLifecycleOptions does NOT accept `tuiSocketPath`
  // (Commit 1/2 frozen). A binds the TUI WS internally and exposes the
  // actual port via `lifecycle.tuiWsPortActual()` after start. B's
  // earlier assembly plumbed `opts.tuiSocketPath` unused — dropped here.
  lifecycle = new GatewayLifecycle({
    backendSocketPath: opts.backendSocketPath,
    socketDir: opts.socketDir,
    backendCapability: opts.backendCapability,
    preflight,
  ...
  // SEAM: A's `GatewayLifecycle` does NOT expose `HumanOwnerCoordinator`
  // publicly (protected — Commit 1/2 frozen). B's earlier design
  // monkey-patched the coordinator's `attachTui`/`detachTui` to signal
  // `scheduler.onOwnerAttachmentChanged()`. That path is unavailable
  // without adding a public seam.
  //
  // FLAG (副指挥/通信龙): I did NOT add a seam on A. The scheduler
  // reads `ownerAttached()` (polls A's public `humanOwnerAttached()`) at
  // every dispatch decision — this preserves correctness. It DOES lose
  // the eager re-kick optimization when a TUI attaches while the scheduler
  // is parked. If runtime evidence in C3 shows measurable latency, we can
  // either (a) add a public read-only accessor
  //   `humanOwnerCoordinator(): HumanOwnerCoordinator | null`
  // on A's lifecycle (requires 通信龙 sign-off — protected file), OR
  // (b) add an event/observer callback on A's coordinator (also
  // protected), OR (c) route the eager-kick through a scheduler tick
  // driven by inbox-pump activity (no A change). Deferred to C3 evidence.
