...
describe("L3-R6 gateway assembly — one socket, one mux, A orchestration + B policy", () => {
  // 副指挥 ad8b1f53 C1 SEAM FLAG: this test drives owner attach via
  // `lifecycle.humanOwnerCoordinator().attachTui()`. A frozen (Commit
  // 1/2, protected) keeps `humanOwner` private and does NOT expose an
  // accessor. Real-path attach requires a WS client to
  // `lifecycle.tuiWsPortActual()` sending a valid TUI lease frame —
  // that harness lands in C2. Skipping keeps compile-first WIP SHA
  // honest; the owner-gate L1 assertion is preserved via the new
  // server-side rfc030-cross-alias-mcp-red.test.ts (real bearer path).
  test.skip("full path: assemble → owner gate → enqueue → turn via A-mux → completion via transport notifications → ledger (C2: needs real WS TUI attach harness)", async () => {
    ...
    handle = await assembleCodexGateway({
      ...paths,
      sqlitePath: ":memory:",
      backendCapability: "test-backend-capability-32-chars",
      spawnFactory: async () => ({ url: app.url, shutdown: () => {} }),
    });

    // A's orchestration is live: state running, backend UDS on disk,
    // TUI on a dynamically-chosen WS port (A owns the port; there is no
    // TUI UDS in frozen A). Thread bootstrapped over the SINGLE mux.
