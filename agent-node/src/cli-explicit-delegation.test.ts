...
describe("extractExplicitDelegation", () => {
  ...
  it("does not match no alias", () => {
    ...
    expect(extractExplicitDelegation("这次改动的价值在于它能让 节点丙 免于一次会丢授权的重启,请按这个方案执行。")).toBeNull();
    expect(extractExplicitDelegation("结论已写在 PR 里,后续如果有疑问请 节点丁 确认一下再合并。")).toBeNull();
    // 但真正的祈使派单(开头 / 行首)照旧
