...
The shared Grok TUI (`grok-build-cli` co-presence) ships on the npm `preview` channel (agent-node ≥ 2.5.0-preview.43 with agent-network ≥ 2.3.0-preview.59); `latest` does not include it. Development branches may contain source-only experiments, but they are not an installable product path until published and listed in the [Grok TUI status page](https://anet.sh/guide/grok-copresence).
