...
api_curl() { curl -s -H "Authorization: Bearer ${AUTH_TOKEN}" "$@"; }
mcp_curl() { curl -s -X POST http://127.0.0.1:9200/mcp -H "Authorization: Bearer ${AUTH_TOKEN}" "$@"; }
...
sleep 5
api_curl http://127.0.0.1:9200/api/status | python3 -c "
import sys,json
...
sleep 1
TASKS_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$TASK_ID" 2>/dev/null)
...
TASKS_BY_STATUS=$(api_curl "http://127.0.0.1:9200/api/tasks?status=replied" 2>/dev/null)
...
mcp_call() {
  ...
  if [ -z "$AUTH_TOKEN" ]; then
    echo "[mcp_call] AUTH_TOKEN is empty" >&2
    return 1
  fi
  echo "$AUTH_TOKEN" >/dev/null
  timeout 10 curl -s -X POST http://127.0.0.1:9200/mcp \
...
sleep 1
FAIL_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$FAIL_TID" 2>/dev/null)
...
FROM_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?from_name=v2-tester" 2>/dev/null)
echo "$FROM_CHECK" | grep -q '"ok":true' && pass "tasks filter by from_name" || fail "from_name filter broken"
TO_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?to_name=e2e-agent&limit=3" 2>/dev/null)
...
COMBO=$(api_curl "http://127.0.0.1:9200/api/tasks?status=delivered&to_name=prio-agent" 2>/dev/null)
...
STATS=$(api_curl "http://127.0.0.1:9200/api/stats" 2>/dev/null)
...
NODES=$(api_curl "http://127.0.0.1:9200/api/nodes" 2>/dev/null)
echo "$NODES" | grep -q '"ok":true' && pass "nodes API works" || fail "nodes API broken"
echo ""

# 23. messages REST API
echo "23. Testing messages API..."
MSGS=$(api_curl "http://127.0.0.1:9200/api/messages?limit=5" 2>/dev/null)
...
wait $PIDS
sleep 1
CONC_COUNT=$(api_curl http://127.0.0.1:9200/api/status 2>/dev/null | python3 -c "
import sys,json
data=json.load(sys.stdin)
...
" 2>/dev/null)
if [ -n "$LARGE_TID" ]; then
  LARGE_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$LARGE_TID" 2>/dev/null | python3 -c "
import sys,json
...
EVENTS=$(api_curl "http://127.0.0.1:9200/api/task_events?task_id=$TASK_ID" 2>/dev/null)
...
sleep 1
# Verify failed
RETRY_C1=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$RETRY_TID" 2>/dev/null)
...
sleep 1
RETRY_C2=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$RETRY_TID" 2>/dev/null)
...
# REST stats
REST_STATS=$(api_curl "http://127.0.0.1:9200/api/tasks?limit=1" 2>/dev/null)
...
CANCEL_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$CANCEL_TID" 2>/dev/null)
...
RA_CHECK=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$RA_TID" 2>/dev/null)
...
EXP_C1=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$EXP_TID" 2>/dev/null)
...
EXP_C2=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$EXP_TID" 2>/dev/null)
...
# Verify it's idle
STOP_BEFORE=$(api_curl "http://127.0.0.1:9200/api/status" 2>/dev/null | python3 -c "
import sys,json
data=json.load(sys.stdin)
...
STOP_AFTER=$(api_curl "http://127.0.0.1:9200/api/status" 2>/dev/null | python3 -c "
import sys,json
data=json.load(sys.stdin)
s = next((s for s in data['sessions'] if s['alias']=='stop-verify'), None)
print(s['status'] if s else 'not_found')
...
SIM_CHECK1=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$SIM_TID" 2>/dev/null)
...
# Verify task = acked
SIM_CHECK2=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$SIM_TID" 2>/dev/null)
...
# Verify task = running
sleep 1
SIM_CHECK3=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$SIM_TID" 2>/dev/null)
...
SIM_CHECK4=$(api_curl "http://127.0.0.1:9200/api/tasks?task_id=$SIM_TID" 2>/dev/null)
...
SIM_EVENTS=$(api_curl "http://127.0.0.1:9200/api/task_events?task_id=$SIM_TID" 2>/dev/null)
...
STATUS2=$(api_curl http://127.0.0.1:9200/api/status 2>/dev/null)
