...
[ -n "$OWNER_TOKEN" ] && [ -n "$MEMBER_TOKEN" ] && [ -n "$VIEWER_TOKEN" ] && pass "three users registered/login" || { fail "user setup failed"; exit 1; }
echo ""

echo "2. Owner creates network and invites member/viewer..."
...
echo "$INV_MEMBER" | grep -q '"ok":true' && echo "$INV_VIEWER" | grep -q '"ok":true' && pass "owner creates invites" || fail "owner invite failed"
...
echo "$JOIN_MEMBER" | grep -q '"ok":true' && echo "$JOIN_VIEWER" | grep -q '"ok":true' && pass "member/viewer join network" || fail "join failed"
...
echo "$OWNER_NODE_TOKEN_RES" | grep -q '"ok":true' && echo "$MEMBER_NODE_TOKEN_RES" | grep -q '"ok":true' && echo "$VIEWER_NODE_TOKEN_RES" | grep -q '"ok":false' && [ -n "$OWNER_NODE_MCP_TOKEN" ] && [ -n "$MEMBER_NODE_MCP_TOKEN" ] && [ -z "$VIEWER_NODE_MCP_TOKEN" ] && pass "node-token permissions enforced" || fail "node-token permission setup failed"
echo ""
...
OWNER_REMOVE=$(curl -s -X DELETE "http://127.0.0.1:9200/api/networks/$NET_ID/members/$VIEWER_ID" \
  -H "Authorization: Bearer $OWNER_TOKEN")
