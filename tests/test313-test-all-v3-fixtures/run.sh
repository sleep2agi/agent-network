...
run_case() {
  local name=$1
  local script=$2
  local home="/tmp/test313-${name}-home"
  local rc=0
...
echo "source_commit=$TEST313_SOURCE_COMMIT"
case "${TEST313_MUTATION:-none}" in
  none) ;;
  drop-node-registration)
    sed -i '/^register_agent agent-[ab] /d' /app/test-networks.sh
    ;;
  drop-login-bootstrap)
    sed -i '/^anet login .*test313-login\.log$/d' /app/test-config.sh
    ;;
  *) echo "FAIL: unknown TEST313_MUTATION"; exit 2 ;;
esac

case "${TEST313_CASE:-all}" in
  ...
  all)
    ...
    exit 2
    ;;
  *) echo "FAIL: unknown TEST313_CASE"; exit 2 ;;
esac

case "${TEST313_EXPECT:-green}" in
green)
  rc=$(cat "$ARTIFACT_DIR/${TEST313_CASE}.rc")
  [[ "$rc" -eq 0 ]] || { cat "$ARTIFACT_DIR/${TEST313_CASE}.log"; echo "FAIL: repaired script returned $rc"; exit 1; }
  grep -q "Results:" "$ARTIFACT_DIR/${TEST313_CASE}.log" \
    || { echo "FAIL: repaired script did not finish its assertions"; exit 1; }
  grep -q '❌' "$ARTIFACT_DIR/${TEST313_CASE}.log" \
    && { echo "FAIL: repaired script reported a failed assertion"; exit 1; }
  echo "RESULT: PASS"
  ;;
red)
if [[ "${TEST313_CASE}" == networks ]]; then
...
;;
*) echo "FAIL: unknown TEST313_EXPECT"; exit 2 ;;
esac
