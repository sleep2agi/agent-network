...
anet login --hub http://127.0.0.1:9200 --username cfg-user --password cfg-test-password >/tmp/test313-login.log
python3 -c 'import json,os
c=json.load(open(os.path.join(os.environ["HOME"],".anet/config.json"))); assert c.get("token") and c.get("network_id")'
anet node create cfg-test --runtime codex-sdk --model test-model >/tmp/test313-create.log
...
HUB=$(timeout -k 1 3 agent-node --alias cfg-fb --config .anet/nodes/cfg-test/config.json 2>&1 | grep "hub:" || true)
...
HUB2=$(timeout -k 1 3 agent-node --alias cfg-proj --config .anet/nodes/cfg-test/config.json 2>&1 | grep "hub:" || true)
...
HUB3=$(COMMHUB_URL=http://env:9200 timeout -k 1 3 agent-node --alias cfg-env --config .anet/nodes/cfg-test/config.json 2>&1 | grep "hub:" || true)
echo "$HUB3" | grep -q "env:9200" && pass "COMMHUB_URL env overrides project" || fail "env hub: $HUB3"
RT=$(RUNTIME=claude-agent-sdk timeout -k 1 3 agent-node --alias cfg-rt --config .anet/nodes/cfg-test/config.json 2>&1 | grep "runtime:" || true)
echo "$RT" | grep -q "claude-agent-sdk" && pass "RUNTIME env overrides config" || fail "runtime: $RT"
MD=$(MODEL=env-model timeout -k 1 3 agent-node --alias cfg-md --config .anet/nodes/cfg-test/config.json 2>&1 | grep "model:" || true)
echo "$MD" | grep -q "env-model" && pass "MODEL env overrides config" || fail "model: $MD"
echo ""

# 4. CLI overrides env
echo "4. CLI overrides env..."
HUB4=$(COMMHUB_URL=http://env:9200 timeout -k 1 3 agent-node --alias cfg-cli --config .anet/nodes/cfg-test/config.json --hub http://cli:9200 2>&1 | grep "hub:" || true)
echo "$HUB4" | grep -q "cli:9200" && pass "CLI --hub overrides env" || fail "cli hub: $HUB4"
RT2=$(RUNTIME=codex-sdk timeout -k 1 3 agent-node --alias cfg-clirt --config .anet/nodes/cfg-test/config.json --runtime claude-agent-sdk 2>&1 | grep "runtime:" || true)
echo "$RT2" | grep -q "claude-agent-sdk" && pass "CLI --runtime overrides env" || fail "cli rt: $RT2"
MD2=$(MODEL=env-model timeout -k 1 3 agent-node --alias cfg-climd --config .anet/nodes/cfg-test/config.json --model cli-model 2>&1 | grep "model:" || true)
...
TK=$(timeout -k 1 3 agent-node --alias cfg-tk --config .anet/nodes/cfg-test/config.json 2>&1 | grep "auth" || true)
...
TK2=$(timeout -k 1 3 agent-node --alias cfg-tkfb --config .anet/nodes/cfg-test/config.json 2>&1 | grep "auth" || true)
echo "$TK2" | grep -q "(auth)" && pass "token fallback to global" || fail "token fb: $TK2"
# Env overrides both
TK3=$(COMMHUB_TOKEN=env-tok timeout -k 1 3 agent-node --alias cfg-tkenv --config .anet/nodes/cfg-test/config.json 2>&1 | grep "auth" || true)
echo "$TK3" | grep -q "(auth)" && pass "COMMHUB_TOKEN env overrides all" || fail "token env: $TK3"
echo ""

# 6. Alias priority
echo "6. Alias priority..."
AL=$(COMMHUB_ALIAS=env-alias timeout -k 1 3 agent-node --alias cli-alias --config .anet/nodes/cfg-test/config.json 2>&1 | head -5 || true)
...
DF=$(timeout -k 1 3 agent-node --alias def-test 2>&1 | head -8 || true)
...
cd /tmp/cfg-test
anet login --hub http://127.0.0.1:9200 --username cfg-user --password cfg-test-password >/tmp/test313-relogin.log
GLOBAL_TOKEN=$(python3 -c 'import json,os; print(json.load(open(os.path.join(os.environ["HOME"],".anet/config.json")))["token"])')
anet node create inherit-test --runtime codex-sdk >/tmp/test313-inherit-create.log
