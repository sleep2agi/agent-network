...
# 变了要人来确认是真变了还是扫漏了。
# 这三个数随 #831 的符号锚点改造逐批下降:106/70/141 → 106/53/107 → 106/27/53 → 106/18/35。
...
[[ "$uniq"  -eq 18  ]] || fail "预期 18 个唯一 pin,实际 $uniq"
[[ "$occ"   -eq 35 ]] || fail "预期 35 处原始出现,实际 $occ"
echo "  OK  walk 路径与 git 路径给出同一份清单(106 文件 / 18 唯一 pin / 35 处)"
