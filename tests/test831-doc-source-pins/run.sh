...
#
# 2026-08-24:110 → 111。#1160 新增桌面自动更新的静态 JSON 清单:
#     docs-site/docs/public/desktop/update/latest.json
# checker 有意扫描 JSON(源码行号链接也可能藏在结构化文档中),所以这份公开清单
# 属于取集范围,不应排除。它不含 `blob/<ref>/<path>#L<n>` 链接,因此仍然只有
# `files` 增加 1,`uniq`=11 与 `occ`=28 均不变。
