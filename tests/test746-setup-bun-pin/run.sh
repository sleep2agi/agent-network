...
if len(found) != 5:
    print(f"FAIL: expected exactly 5 setup-bun occurrences, found {len(found)}", file=sys.stderr)
    sys.exit(1)
if bad:
...
cat "$BASELINE_LOG"
grep -Fxq 'setup_bun_occurrences=5 expected_version=1.3.14' "$BASELINE_LOG"
