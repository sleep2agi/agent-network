...
function assertOwnedRegular(path, expectedMode, label, allowMissing = false, repair = false) {
  ...
  if (mode !== expectedMode) {
    ...
    if (repair) chmodSync(path, expectedMode);
  }
}
function assertOwnedDirectory(path, expectedMode, label, repair = false) {
  const st = lstatSync(path);
...
if (!["plan", "apply"].includes(args.mode)) fail("--mode must be plan or apply");
...
assertOwnedRegular(configPath, 0o600, "config", false, args.mode === "apply");
assertOwnedDirectory(inventoryDir, 0o700, "inventory directory", args.mode === "apply");
...
if (existsSync(goalsRoot)) assertOwnedDirectory(goalsRoot, 0o700, "goals root", args.mode === "apply");
