...
const BOOLEAN_FLAGS = new Set(["--all", "--tmux", "--new-session", "--grok-headless"]);
...
This wizard creates one agent node for this project:
  ...
  - runtime: claude-code-cli / codex-sdk / claude-agent-sdk / grok-build-acp / grok-build-cli
  ...
  try {
    ...
    pickedRuntime = await sel({
      ...
      choices: [
        ...
        { value: "grok-build-cli",   name: "grok-build-cli   — EXPERIMENTAL 共存 TUI，仅可信任务" },
...
async function createCommand(idOverride?: string) {
  ...
  if (id.startsWith("--")) {
    console.error("Usage: anet node create <node-name> [--runtime claude-code-cli|codex-sdk|claude-agent-sdk|grok-build-acp|grok-build-cli] [--model ...] [--tools ...]");
  ...
  const runtimeAlreadyExplicit = explicitRuntime === "codex-sdk" || explicitRuntime === "claude-code-cli" || explicitRuntime === "grok-build-acp" || explicitRuntime === "grok-build-cli";
...
async function launchAgent(id: string, forceNewSession = false) {
  ...
  if (
    ...
    } else try { execSync("which agent-node", { stdio: "pipe" }); } catch {
      cmd = "npx";
    ...
    const childEnv = runtime === "grok-build-cli"
