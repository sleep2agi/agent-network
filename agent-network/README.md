...
- **Agent nodes**: long-running AI workers backed by Claude Code, Claude Agent SDK, Codex SDK, Grok Build CLI, or Grok Build ACP.
...
1. Runtime: `claude-code-cli`, `claude-agent-sdk`, `codex-sdk`, `grok-build-cli`, or `grok-build-acp`.
2. Provider preset: Anthropic, MiniMax, InternLM, Xiaomi MiMo, or `custom` (any Anthropic-compatible endpoint — used for DeepSeek / GLM / Kimi / OpenRouter etc.; `codex-sdk` for OpenAI Codex; opt-in `grok-build-cli` or `grok-build-acp` for xAI Grok).
...
| `grok-build-cli` | You want one Grok TUI shared by a human and CommHub tasks | Source-only and unpublished; new profiles use Option A2 co-presence and `anet grok attach`, while old profiles stay headless |
...
The current source tree provides `grok-build-cli` as an explicit opt-in runtime. It has not been published to an npm dist-tag, and it does not replace or deprecate `grok-build-acp`. A newly created profile defaults to Option A2 co-presence: `agent-node` owns one live Grok TUI through `node-pty`, and a human terminal connects to its local proxy.
...
# Run from another terminal; Ctrl-] detaches without stopping the node
anet grok attach grok-demo
```

For another project, set `ANET_AGENT_NODE_BIN=/absolute/path/to/agent-node/dist/cli.js`. `anet setup` does not auto-install the npm stable agent-node that lacks this source-only implementation. Startup requires the probed `grok 0.2.93 (f00f96316d)` build exactly and checks its interactive/session/leader-socket/sandbox and human-approval capabilities before accepting work.

`anet node create` writes `grokCopresence: true`, `grokLeaderSocket`, and `grokAttachSocket`. Both sockets use short opaque paths in an owner-only runtime directory and must be current-uid, non-symlink Unix sockets. `grokCliSession` remains independent of the ACP runtime's `grokSession`; a lifetime `(leader socket, session ID)` lock rejects a second bridge.

The single input arbiter gives human editing and human turns priority. CommHub tasks wait FIFO and are injected visibly as `[Agent Network/from=<sender>/task=<task-id>] <message>`. A prefix is trusted only after that task ID was registered from SSE. The bridge tails Grok's cwd/session JSONL, waits for terminal `turn_ended` rather than the first assistant/tool record, then maps the final assistant text back to the registered task. Human turns are never returned as network replies.

Option A2 keeps CommHub authority in `agent-node`: the Grok TUI receives no real ntok, CommHub environment, or CommHub MCP server, and the wrapper does not create a project `.mcp.json` identity. Unprefixed human prompts are considered only by the explicit-delegation parser. Clear instructions such as `给 <alias> 发任务: ...` or `send_task <alias> ...` are dispatched by the wrapper; ordinary TUI conversation is not forwarded.

The attached human owns every approval. Permission events are correlated by request ID when present, otherwise by tool identity, to one proxied human action; an automatic resolve is fatal. The approval UI accepts only Enter (pinned to allow-once) or Ctrl-C. Persistent-grant digits/navigation, Ctrl+O, Shift+Tab, and slash commands are blocked, and an observed YOLO/auto transition shuts the runtime down. Every spawn rebuilds and reinspects the isolated policy, while project `.grok`, `.claude`, and `.mcp.json` stay sandbox-denied even if absent. A normal PTY failure resumes only the same cwd/session without headless fallback or active-turn replay; a failure while approval is pending is terminal.

Profiles that predate co-presence and lack `grokCopresence: true` retain the old per-task prompt-file/streaming-JSON runtime and cannot use `anet grok attach`; they are not silently migrated. Pass `--grok-headless` when creating a new node to select that compatibility behavior deliberately.

The co-presence Docker suite is `tests/test219-grok-copresence/`, with its saved result in `docs/tests/report-test219.txt`. The existing `grok-build-acp` runtime and its config/session behavior remain unchanged and are not deprecated.
...
- Grok Build CLI / ACP runtime hardening
