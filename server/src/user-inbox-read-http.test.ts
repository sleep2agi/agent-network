...
beforeAll(async () => {
  ...
  // #1493 —— network_id 升到 schema 级 NOT NULL 后,「network_id 为 NULL 的行」已
  // **schema 级不可能**(写路径原本三闸挡、现在 DB 直接拒)。原先手播的 dm_a_nonet /
  // dm_admin_nonet 两条 NULL 夹具随之删除:
  //   · dm_a_nonet(证"受网络限制成员读不到 NULL 行")—— 网络 scope 隔离已由下面
  //     dm_a_foreign_net(A 自己的行、落在 A 非成员的网)覆盖,不丢覆盖。
...
describe("#1459 ① P3 GET /api/messages?scope=user", () => {
  ...
  // #1493 后 NULL 网络行不可能,原"受网络限制成员读不到 NULL 行"一测删除——
  // 网络 scope 隔离已由上面 dm_a_foreign_net(承重断言,test 112 line 121)覆盖。
  // admin 一测重锚到非 NULL:验 admin(scope 助手 no-op)**仍只读自己的、不绕过 user_id 过滤**。
