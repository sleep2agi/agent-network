...
#   curl -fsSL https://anet.vansin.me/hub-only.sh | NO_DASHBOARD=1 MINIMAX_KEY=sk-cp-xxx bash
#
#   # 想自启 (重启服务器自动拉): 加 AUTOSTART=1
#   curl -fsSL https://anet.vansin.me/hub-only.sh | AUTOSTART=1 MINIMAX_KEY=sk-cp-xxx bash
#
#   # 清状态重来: 加 WIPE=1 (删 ~/.anet ~/.commhub + 杀进程)
#   curl -fsSL https://anet.vansin.me/hub-only.sh | WIPE=1 MINIMAX_KEY=sk-cp-xxx bash
#
...
set +e
npm i -g @sleep2agi/agent-network@preview @sleep2agi/agent-node@preview >/dev/null 2>&1
RC=$?
if [ $RC -ne 0 ]; then
