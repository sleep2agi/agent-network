...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--runtime` | 不传则走交互式 **runtime-first wizard** ([#133](https://github.com/sleep2agi/agent-network/issues/133) v0.9.2 起): 5-way 选 `claude-agent-sdk` / `claude-code-cli` / `codex-sdk` / `grok-build-acp` / `opencode-cli`. **只有 `claude-agent-sdk` 才继续走 vendor picker + model 选择 + API Key**; `claude-code-cli` / `codex-sdk` / `grok-build-acp` print 对应 CLI 的 auth login hint 然后跳过 vendor (`claude auth login` / `codex auth login` / `grok auth login`；`grok-build-acp` 另需 `GROK_CODE_XAI_API_KEY` env)；`opencode-cli` 需先装 `opencode` CLI，再选 vendor preset (anthropic / openai，key 从 env 读)。详见 [runtimes](/guide/runtimes)。详细 wizard 顺序见 [上手指南 §5](/guide/getting-started#_5-创建-agent-节点). | `claude-agent-sdk` / `codex-sdk` / `claude-code-cli` / `grok-build-acp` / `opencode-cli` |
