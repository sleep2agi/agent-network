...
本页是**新用户首次安装**当前 stable（v0.10.6）端到端跑通的最小路径。每一步都经过 Playwright + Docker E2E 验证，照着敲就能走通。

::: tip 已经装过 anet？
**升级**走 [升级指南](/guide/upgrade) —— 一行 `anet upgrade` 自动 picks up 4 个 npm 包 latest（含 v0.10.1 hotfix + v0.10.2 Hero A/D + v0.10.3 codex-sdk gpt-5.5 + v0.10.4 dashboard orphan-band + v0.10.5 batch wizard 双修 + **v0.10.6 [#154 anet upgrade Option B detached spawn 默认开启](https://github.com/sleep2agi/agent-network/issues/154) + [#155 batch wizard silent-exit 修](https://github.com/sleep2agi/agent-network/issues/155)**）+ 用 `anet project restart` 重启 cwd 节点接新版。

⚠ **chicken-and-egg 注**：当前 2.2.4 及以下 binary 跑 `anet upgrade` 仍是旧 "skipped" 行为，**只需手装 1 次** `npm install -g @sleep2agi/agent-network@2.2.5` 升到 2.2.5+，之后再升级都自动 detached spawn。详见 [升级指南 chicken-and-egg 段](/guide/upgrade)。本页是给**全新机器** / 从来没装过 anet 的用户（不受这个限制 —— 首次 `npm install -g` 直接拉 latest）。
:::

::: tip 组件职责（v0.10.6 stable）
