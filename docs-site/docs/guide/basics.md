...
::: tip 这一页是写给谁的？
如果你看到 "CLI"、"Server"、"Token"、"MCP" 这些词不太确定是什么意思，花 3 分钟看完这一页，后面的文档就不会卡壳了。整体定位见 [5 分钟懂 anet](/guide/introduction)。
:::
...
- **能力**：由它使用的 AI 模型决定（Codex (codex-sdk)、Claude、MiniMax 等）
...
::: info 验证成功
登录成功后，运行 `anet whoami` 应该看到类似输出（verify [`cli.ts whoamiCommand`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts)）：
```
...
**系统级 role vs 网络级 role**：`whoami` 的 `Role:` 字段是系统级 `users.role`（仅 `admin` / `user`），**不是**当前 network 内的 `owner/admin/member/viewer`。查 network role 跑 `anet network members` 看自己那行。详见 [角色常见问题](/guide/account-system#role-faq)。
:::
