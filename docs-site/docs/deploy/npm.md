...
| `@sleep2agi/agent-node` | `agent-node` | Agent 运行时，驱动 **2 个 SDK runtime**：`claude-agent-sdk` / `codex-sdk`（`claude-code-cli` runtime 不走 agent-node —— 直接 spawn 本机 `claude` 二进制） | regular dep `@anthropic-ai/claude-agent-sdk`；`@openai/codex-sdk` optionalDependency |
...
# 3. 创建并启动 Agent
anet node create assistant --runtime claude-code-cli   # 最省事：复用 Claude 订阅、0 配置（需本机装 claude：npm i -g @anthropic-ai/claude-code）
