...
创建能力:未知 —— 这台 daemon 没报过这一格(agent-node 版本早于 preview.55)。升级后必须重启 daemon 才生效。
创建能力:查不到 —— hub 上没有这个 node_id(还没注册过，或注册到了别的网络)
创建能力:查不到 —— hub 拒绝了本机的身份(HTTP 401)。
  hub 是通的,要修的是**凭据**不是网络:跑 anet login
```
...
🔴 **启动时若看到「装了但 PATH 上找不到」的警告，先修它再往下走。**
`anet daemon start` 会检查：这台 daemon 声称支持的 runtime，它们依赖的二进制是不是
在 PATH 上找得到。**只在「装了、却不在 PATH 上」时才出声** —— 因为
**daemon 建出来的子节点继承 daemon 的 PATH**，这种情况下子节点会报
「xxx CLI not found」，而那个东西其实装着，照着报错去重装不会有任何效果。
（压根没装时不warn：那种情况节点启动时的报错说得更细。）
