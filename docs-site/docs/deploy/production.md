...
::: danger 默认配置不能直接挂公网
当前 stable（v0.10.8，对应 `commhub-server` semver；旧的 `v2.1.x` CLI 版本号体系已废弃，见 [changelog](/changelog)）的默认配置只为**本机使用**优化。直接 `--host 0.0.0.0` 公网开放 = 给攻击者敞开大门。
...
- **v0.10.2 → v0.10.8 已 ship**（同日 7 patch chain，[changelog](/changelog)）：host disk telemetry ✅（v0.10.2 Hero A `df -k` POSIX 标准）+ dashboard Hero D 拓扑前缀 Option C + disk render（v0.10.2）+ codex-sdk default model `gpt-5.5` + yolo flags 写进 config（v0.10.3 [#149](https://github.com/sleep2agi/agent-network/issues/149)）+ dashboard 拓扑 orphan-band "其他" cluster + `anet upgrade` UX warning（v0.10.4 [#150](https://github.com/sleep2agi/agent-network/issues/150) + [#151](https://github.com/sleep2agi/agent-network/issues/151)）+ `anet create --batch` wizard workdir mode + codex/claude skip API key prompt（v0.10.5 [#152](https://github.com/sleep2agi/agent-network/issues/152) + [#153](https://github.com/sleep2agi/agent-network/issues/153)）+ `anet upgrade` Option B detached spawn 默认 + batch wizard silent-exit 修（v0.10.6 [#154](https://github.com/sleep2agi/agent-network/issues/154) + [#155](https://github.com/sleep2agi/agent-network/issues/155)）+ codex-sdk batch path yolo flags parity + `--no-yolo` opt-out for CI/scripted（v0.10.7 [#156](https://github.com/sleep2agi/agent-network/issues/156)）+ Dashboard Servers 面板 UI 文案修 + R502 TopoGraph density tier polish（v0.10.8 [#157](https://github.com/sleep2agi/agent-network/issues/157)）—— **Cycle 11-13 累计 18 个 `@latest` publish 0 split-brain / 0 rollback / 0 retry**
