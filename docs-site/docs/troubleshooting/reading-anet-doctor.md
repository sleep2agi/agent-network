...
## 飞书 × runtime 那两行（2.3.0-preview.76+）

配了飞书通道、而 runtime 不是 `claude-agent-sdk` 的节点，会各打一行：
...
**它说的是「这条组合没有被验证过」，不是「它坏了」。** 要不要改，取决于
这个节点上飞书通道会不会真的被用到。

结尾还有一行**无论有没有命中都会打印**的盘点：

```
Feishu × runtime 盘点 —— 扫了 <目录> 下 N 个节点配置；
这只是**这一棵 .anet 树**，同一台机器上可能还有别的
```

🔴 **这一行的分母比结论更重要。** 只在出事时说话的检查，验不了自己 ——
有了 `N`，「0 个命中」和「一个节点都没扫到」才区分得开。

🔴 **`0 个命中` 不等于这台机器上没有。** doctor 扫的是
`<当前工作目录>/.anet/nodes`，**只有这一棵树**。同一台机器上别处起的节点
它够不着 —— 换个目录再跑一次才算覆盖到那一棵。
