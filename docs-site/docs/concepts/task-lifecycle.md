...
R231 校准：原 doc 写 `anet tasks --detail t_xxx` CLI 命令不存在（[`cli.ts:2960-2994 tasksCommand`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L2960) 只解析 `--status` 和 `--limit`，没 `--detail` 参数），用户跑会 hit `?` 占位输出。
