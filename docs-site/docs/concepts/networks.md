...
::: info Dashboard 0.4.6 实际行为（v0.10.0 stable Phase 1）
角色 → 按钮可见性的 UI 联动**部分实装**。即使按钮当前还显示，**Server 端会 403 拒绝**（`canWrite()` 强制 RBAC），权限本身不会绕过。完整 UI 按钮 hiding **v0.9.x + v0.10.0 scope 都未动**（v0.9.x Recovery & Observability、v0.10.0 Direct Runtime + Observability Foundations 主题为先），dashboard `0.5.0` Phase 2（N 站马 §3.D/F/G ship 后 promote）跟 v0.11+ Dashboard 改造里再补。
:::
...
下表保留为自部署管理员**手动设置软配额**的设计参考（v0.9.x / v0.10.0 都未实现 —— v0.9.x Recovery & Observability、v0.10.0 Direct Runtime + Observability Foundations scope 为先；排到 v0.11+ / 未排期）。
:::
