...
> **v0.10.0 → v0.10.5 全 chain LIVE（2026-05-16/05-17）**：4 包通过 npm `latest` tag ship —— `@sleep2agi/agent-network@2.2.4`（v0.10.5 batch wizard 双修 + v0.10.4 anet upgrade UX + v0.10.3 codex preset + v0.10.1 PINNED hotfix）/ `@sleep2agi/agent-node@2.4.2`（v0.10.3 codex-sdk gpt-5.5 + yolo flags / v0.10.2 Hero A host disk telemetry）/ `@sleep2agi/commhub-server@0.8.2`（v0.10.0 守护节点 endpoints + #142 schema align）/ `@sleep2agi/agent-network-dashboard@0.5.2`（v0.10.4 orphan-band cluster + v0.10.2 Hero D 拓扑前缀标签 + disk render + 100+ 轮 polish R317-R438）。完整变更见 [changelog](/changelog)。

::: tip ✅ v0.10.7 stable 全 4 包 ship 完（含 v0.10.0 Phase 1+2 + v0.10.1 PINNED hotfix + v0.10.2 Hero A+D + v0.10.3 codex preset + v0.10.4 dashboard orphan-band + v0.10.5 batch wizard 双修 + v0.10.6 anet upgrade Option B + v0.10.7 codex-sdk batch yolo parity）
...
| `@sleep2agi/agent-network-dashboard` | `0.5.2` ✅（v0.10.4 orphan-band + v0.10.2 Hero D + disk render + 100+ polish）|
