...
The current stable line (v0.10.13 — the `v0.X.Y` format mirrors `commhub-server`'s `0.X.Y` semver style; the old `v2.1.x` CLI version scheme is deprecated — see [changelog](/en/changelog)) is tuned for **local use only**. Running with `--host 0.0.0.0` straight to the open internet leaves you wide open.
