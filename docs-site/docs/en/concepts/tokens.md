...
| Delete node (`anet node delete`) | Unchanged | **Not auto-revoked** — the `api_tokens` row stays on the hub ([`cli.ts:2564` `notifyServerOffline`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L2564) only sends `report_status` offline; it does not delete the token). Use `anet token revoke <id>` separately to fully clean up. |
