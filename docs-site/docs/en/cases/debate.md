...
- Hack the demo itself? Source is [`demoDebateCommand` in `agent-network/bin/cli.ts` (L3769)](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L3769) (there's no separate `src/commands/demo/` directory — the demo logic all lives in cli.ts)
