...
`whoami`'s `Role:` field shows the **system-level** `users.role` (only `admin` / `user`) — it is **not** your role within the current network (`owner / admin / member / viewer`). To check the per-network role, run `anet network members` and find your own row. See [roles → FAQ](/en/concepts/roles).
:::
...
- Workaround for now: re-run `anet login --username admin --password <new-password>` to refresh `~/.anet/config.json`; admin-utok.json is a one-time bootstrap credential — `config.json` is the authoritative source going forward. **Neither v0.9.x nor any v0.10.0-10 scope addressed this** (v0.9.x Recovery & Observability, v0.10.0 Direct Runtime + Observability Foundations, v0.10.1-8 PINNED hotfix / Hero A+D / Servers UI copy fix / 8-cycle wizard + UX-fix chain themes took priority; v0.10.9 codex-sdk image input + commhub attachment `meta_json`; v0.10.10 Xiaomi MiMo 5-model preset + envRef wizard-to-start auto-source); a proper auto-sync fix (passwd-time refresh of `admin-utok.json`) is queued for v0.11+ / unscheduled.
