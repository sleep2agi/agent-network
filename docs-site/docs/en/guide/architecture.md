...
Set `ANET_CODEX_STDIO_DIRECT=1` to make agent-node switch the codex runtime from the `@openai/codex-sdk` wrapper to **`spawn('codex', ['app-server'])` + a direct stdio JSON-RPC client**, getting the full protocol surface (thread / turn / item / realtime) and **bypassing** the wrapper's `--mcp-config` HTTP-transport bug family ([#102](https://github.com/sleep2agi/agent-network/issues/102) hang root cause). The current default path follows source and release notes. Codex built-in tools still come from the codex CLI; CommHub tools are injected by agent-node per node and can actively issue `send_task`. Details: [runtimes — codex-sdk § codex-direct-stdio](/en/guide/runtimes#codex-sdk) + [agent-node — env vars § ANET_CODEX_STDIO_DIRECT](/en/guide/agent-node#environment-variables) + [v0.10.0 GitHub release notes](https://github.com/sleep2agi/agent-network/releases/tag/v0.10.0).
:::
...
> ⚠ Debug tip: if the LLM can't call a commhub tool, check the runtime first — for `claude-agent-sdk` nodes, confirm `commhub-mcp.ts` is in dist; for `claude-code-cli` nodes, check the `.mcp.json` has `type: stdio` and the `.anet/node-server.js` path is correct; for `codex-sdk` nodes, look at the agent-node parent process logs and confirm per-node CommHub tool injection; for `grok-build-acp` nodes, look for `[grok] commhub MCP server resolved: <abs-path>` in the agent-node log plus the per-node isolated cwd under `.anet/nodes/<alias>/`. See [grok-build-runtime.md](https://github.com/sleep2agi/agent-network/blob/main/docs/grok-build-runtime.md) and [Connectivity / Channels / MCP](/en/troubleshooting/connectivity-channels-mcp).
