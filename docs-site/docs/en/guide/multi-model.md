...
| **xAI Grok Build** | `grok-build-acp` | ACP-protocol cross-agent collaboration via the local `grok` binary; selectable from the `anet node create` 4-way runtime picker (since v0.10.8 / v0.10.11) — [detailed runtime guide ↗](https://github.com/sleep2agi/agent-network/blob/main/docs/grok-build-runtime.md). Auth: `grok auth login` done + `GROK_CODE_XAI_API_KEY` env (runtime prereq). | Medium |
