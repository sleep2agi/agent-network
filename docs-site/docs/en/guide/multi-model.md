...
> Verification model (post-#104-B): cli.ts no longer uses the old "`MODEL_PRESETS` + `[UNVERIFIED]` tag" scheme — it's now a single [`VENDORS` list (cli.ts:1114-1175)](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1114), and **being in the list means verified-with-real-call**. **Providers that haven't passed verification (DeepSeek / GLM / Kimi) are intentionally NOT in the list** — reach them via the `custom` vendor below (any Anthropic-compatible API works through `custom`). Full detail: [runtimes — Verified vs not](/en/guide/runtimes#verified-vs-not).
