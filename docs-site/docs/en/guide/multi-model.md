...
| **DeepSeek** | `claude-agent-sdk` | ✅ verified — `deepseek-v4-pro` (default) / `deepseek-v4-flash`, low cost (`api.deepseek.com/anthropic`) | Very low |
...
| **xAI Grok Build** | `grok-build-acp` | ACP-protocol cross-agent collaboration via the local `grok` binary; selectable from the `anet node create` 4-way runtime picker (since v0.10.8 / v0.10.11) — [detailed runtime guide ↗](https://github.com/sleep2agi/agent-network/blob/main/docs/grok-build-runtime.md). Auth: `grok login` done + `GROK_CODE_XAI_API_KEY` env (runtime prereq). | Medium |

> Verification model (post-#104-B): cli.ts no longer uses the old "`MODEL_PRESETS` + `[UNVERIFIED]` tag" scheme — it's now a single [`VENDORS` list (in cli.ts, grep `const VENDORS`)](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts), and **being in the list means verified-with-real-call**. **Providers not yet built in (GLM / Kimi / OpenRouter) are reached via the `custom` vendor below** (any Anthropic-compatible API works through `custom`; DeepSeek is now built in — no need for `custom`). Full detail: [runtimes — Verified vs not](/en/guide/runtimes#verified-vs-not).
