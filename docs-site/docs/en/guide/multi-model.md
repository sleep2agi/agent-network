...
| **xAI Grok Build** | `grok-build-acp` | ACP-protocol cross-agent collaboration via the local `grok` binary; currently only enabled via the `--runtime grok-build-acp` flag (not in the `anet node create` 3-way wizard picker yet) — [detailed runtime guide ↗](https://github.com/sleep2agi/agent-network/blob/main/docs/grok-build-runtime.md). Auth: `grok` CLI logged in + `XAI_API_KEY` env. | Medium |
