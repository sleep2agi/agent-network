...
> **✅ verified / ⚠ unverified**: matches the `[UNVERIFIED]` tags in [`cli.ts:1311-1319`](https://github.com/sleep2agi/agent-network/blob/main/agent-network/bin/cli.ts#L1311) `MODEL_PRESETS`. ⚠ unverified providers have their endpoint URLs filled in but no real-API regression has been run yet (R290 chain) — usable, but verify on your own first. Full detail: [runtimes — Verified vs not](/en/guide/runtimes#verified-vs-not).
