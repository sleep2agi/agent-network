...
<!-- version-claim: package=agent-network channel=preview version=2.3.0-preview.92 -->
...
| `2.3.0-preview.92` (current `preview`) | Also **random** — not re-measured per version: `server/src/auth.ts`, which generates it, has had **zero commits** since `.49` shipped (2026-08-27T02:25Z); the logic is byte-identical |
