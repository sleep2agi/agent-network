...
This page is the **first-time install** path for current stable (v0.10.6) — every step is Playwright + Docker E2E verified. The flow follows the v2/v3 Docker + Playwright E2E path: install CLI, start Hub, start Dashboard, log in, create a node, start it.

::: tip Already have anet installed?
**Upgrades** go through the [Upgrade Guide](/en/guide/upgrade) — a single `anet upgrade` picks up all four npm packages at `@latest` (including the v0.10.1 hotfix + v0.10.2 Hero A/D + v0.10.3 codex-sdk gpt-5.5 + v0.10.4 dashboard orphan-band + v0.10.5 batch wizard double-fix + **v0.10.6 [#154 anet upgrade Option B detached spawn enabled by default](https://github.com/sleep2agi/agent-network/issues/154) + [#155 batch wizard silent-exit fix](https://github.com/sleep2agi/agent-network/issues/155)**), followed by `anet project restart` to restart cwd nodes against the new version.

⚠ **Chicken-and-egg note**: if your current binary is 2.2.4 or older, `anet upgrade` still falls back to the old "skipped" behavior — **a one-time manual install** of `npm install -g @sleep2agi/agent-network@2.2.5` is needed to upgrade to 2.2.5+; subsequent upgrades will auto-detached-spawn. See the [Upgrade Guide chicken-and-egg section](/en/guide/upgrade). This page is for **brand-new machines** / users who have never installed anet (this limitation doesn't apply — your first `npm install -g` already pulls the latest).
:::

::: tip Component responsibilities (v0.10.6 stable)
