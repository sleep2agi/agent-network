...
**Upgrades** go through the [Upgrade Guide](/en/guide/upgrade) — a single `anet upgrade` picks up all four npm packages at `@latest` (including the v0.10.1 hotfix + v0.10.2 Hero A/D + v0.10.3 codex-sdk gpt-5.5 + v0.10.4 dashboard orphan-band + v0.10.5 batch wizard double-fix + v0.10.6 [#154 anet upgrade Option B detached spawn enabled by default](https://github.com/sleep2agi/agent-network/issues/154) + [#155 batch wizard silent-exit fix](https://github.com/sleep2agi/agent-network/issues/155) + v0.10.7 [#156 codex-sdk batch path yolo flags parity](https://github.com/sleep2agi/agent-network/issues/156) + `--no-yolo` opt-out + **v0.10.8 [#157 dashboard Servers panel UI copy fix](https://github.com/sleep2agi/agent-network/issues/157) + R502 TopoGraph density-tier polish**), followed by `anet project restart` to restart cwd nodes against the new version.

⚠ **Chicken-and-egg note**: if your current binary is 2.2.4 or older, `anet upgrade` still falls back to the old "skipped" behavior — **a one-time manual install** of `npm install -g @sleep2agi/agent-network@latest` is needed to upgrade to 2.2.5+; subsequent upgrades will auto-detached-spawn. See the [Upgrade Guide chicken-and-egg section](/en/guide/upgrade). This page is for **brand-new machines** / users who have never installed anet (this limitation doesn't apply — your first `npm install -g` already pulls the latest).
:::

::: tip Component responsibilities (v0.10.8 stable)
...
::: info Verified (v0.10.8 stable — inherits the v2 E2E coverage plus new v0.8 regressions plus the v0.9.2 #129 / #132 / #136 fix chain plus the v0.10.0 release-gate playbook first full run plus the v0.10.1 PINNED_SERVER_VERSION chain-bump default-path fix plus the v0.10.2 Hero A agent-node disk telemetry `df -k` Linux/macOS smoke 3/3 PASS + Hero D dashboard topology prefix-label Option C 4/4 verify + v0.10.3 codex-sdk gpt-5.5 vendor preset + yolo flags + v0.10.4 [#150 orphan-band](https://github.com/sleep2agi/agent-network/issues/150) + [#151 anet upgrade UX](https://github.com/sleep2agi/agent-network/issues/151) + v0.10.5 [#152 batch wizard workdir](https://github.com/sleep2agi/agent-network/issues/152) + [#153 codex/claude skip API key](https://github.com/sleep2agi/agent-network/issues/153) + v0.10.6 [#154 anet upgrade Option B detached spawn default](https://github.com/sleep2agi/agent-network/issues/154) + [#155 batch wizard silent-exit fix](https://github.com/sleep2agi/agent-network/issues/155) + v0.10.7 [#156 codex-sdk batch path yolo flags parity](https://github.com/sleep2agi/agent-network/issues/156) + `--no-yolo` opt-out + source-grep gate 2 verify + v0.10.8 [#157 Dashboard Servers panel UI copy fix + R502 TopoGraph density-tier polish](https://github.com/sleep2agi/agent-network/issues/157))
