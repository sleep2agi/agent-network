...
| `anet license` | v0.6 legacy. **OSS users don't need to touch this**: v2.2.12 latest now prints `License: Apache-2.0 (open source) / Source: github / Docs: anet.sh` directly (earlier versions misleadingly printed `License: PRO / Expires <date>` — that was the v0.6 trial-table backward-compat echo, since corrected). |
