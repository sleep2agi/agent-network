...
| Parameter | Default | Description |
|------|--------|------|
| `--runtime` | omit it to enter the interactive **runtime-first wizard** ([#133](https://github.com/sleep2agi/agent-network/issues/133) since v0.9.2): a 5-way picker chooses `claude-agent-sdk` / `claude-code-cli` / `codex-sdk` / `grok-build-acp` / `opencode-cli`. **Only `claude-agent-sdk` continues into the vendor picker + model selection + API-key prompt**; `claude-code-cli` / `codex-sdk` / `grok-build-acp` print the matching `auth login` hint and skip the vendor step (`claude auth login` / `codex auth login` / `grok auth login`; `grok-build-acp` also needs the `GROK_CODE_XAI_API_KEY` env); `opencode-cli` prompts you to install the `opencode` CLI, then pick a vendor preset (anthropic / openai, key read from env). See [runtimes](/en/guide/runtimes). For the full wizard order, see [Getting Started §5](/en/guide/getting-started#_5-create-an-agent). | `claude-agent-sdk` / `codex-sdk` / `claude-code-cli` / `grok-build-acp` / `opencode-cli` |
