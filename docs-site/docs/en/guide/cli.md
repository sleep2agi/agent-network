...
| `anet demo sci-team [opts]` | **Research squad**: 1 leader + N-1 workers (default 10, tunable 5-50). **Phase 1 scaffold** — the leader is currently a placeholder echo; real intelligent fan-out is queued for [RFC-008](https://github.com/sleep2agi/agent-network/blob/main/docs/rfcs/RFC-008-multi-agent-team-convention.md) Phase 2 (RFC status: Proposed; #51 was the now-shipped Phase 1 demo issue) |
...
| `anet license` / `anet activate <key>` | v0.6 legacy commands. **OSS users don't need to touch these.** `anet license` now prints `License: Apache-2.0 (open source)` plus a couple of related lines. `anet activate` only matters as a fallback when you hit `license_expired`. Full detail: [troubleshooting — `license_expired`](/en/troubleshooting#license-expired-license-expired-legacy-behavior). |
...
| Parameter | Default | Description |
|------|--------|------|
| `--runtime` | omit it to enter the interactive **runtime-first wizard** ([#133](https://github.com/sleep2agi/agent-network/issues/133) since v0.9.2): a **4-way** picker (latest) chooses `claude-agent-sdk` / `claude-code-cli` / `codex-sdk` / `grok-build-acp` (`opencode-cli` is **preview-only** — install preview to get the 6-way picker (adds codex-app-server + opencode-cli); see [runtimes](/en/guide/runtimes)). **Only `claude-agent-sdk` continues into the vendor picker + model selection + API-key prompt**; `claude-code-cli` / `codex-sdk` / `grok-build-acp` print the matching `auth login` hint and skip the vendor step (`claude auth login` / `codex login` / `grok login`; `grok-build-acp` also needs the `GROK_CODE_XAI_API_KEY` env); `opencode-cli` prompts you to install the `opencode` CLI, then pick a vendor preset (anthropic / openai, key read from env). See [runtimes](/en/guide/runtimes). For the full wizard order, see [Getting Started §5](/en/guide/getting-started#_4-create-and-start-a-node). | `claude-agent-sdk` / `codex-sdk` / `claude-code-cli` / `grok-build-acp` / `opencode-cli` / `codex-app-server` |
