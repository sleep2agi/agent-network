...
| `anet -v` Components | `agent-node v2.4.13` / `commhub-server v0.8.4` / dashboard `v0.6.0` | Each npm package, independently versioned |
...
**For release tracking**: each bundle release on [GitHub releases](https://github.com/sleep2agi/agent-network/releases) spells out the npm package versions that shipped together in that wave. For instance, `v0.10.15` corresponds to anet `2.2.21` / agent-node `2.4.13` / commhub-server `0.8.4` / dashboard `0.6.0`.

## Why both exist

- **npm package versions are independent**: hotfixes can bump just one package (e.g. commhub-server `0.8.4` lands a server-side fix without forcing the anet CLI to upgrade). Each package evolves via semver on its own cadence.
